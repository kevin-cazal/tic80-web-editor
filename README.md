...
- **Autosave**: Your work (code plus all resources — sprites, map, SFX, music, palette) is saved to the browser's local storage on every change and automatically restored after a page reload, so an accidental refresh won't lose progress. It's a single local slot per browser; clearing site data resets it.
